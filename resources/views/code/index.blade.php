            ...
            <div class="component-card">
                ...
                <div class="c_card">
                    ...
                    <div class="overlay">
                        Thông tin hiển thị
                        @if($item->id)
                            <button onclick="showEditCode({{ $item->id }})">xem</button>
                        @else
...
<div class="model" id="createHtml">
    <div class="modelCreateFromNormal">
        ...
        @csrf
            ...
            <input type="hidden" name="category_id" id="" value="25"/>
...
<div class="model" id="showCodeForm">
...
<script>
    function showEditCode(codeId) {
        const showCodeForm = document.getElementById('showCodeForm');
        showCodeForm.style.display = 'block';
        fetch(`/codes/${codeId}`)
        .then(response => response.json())
        .then(data => {
            document.getElementById('code_id').value = data.id;
    ...
    document.addEventListener('DOMContentLoaded', function() {
        document.getElementById('delete-Code').onsubmit = function(event) {
            ...
            const codeIdElement = document.getElementById('code_id');
            const codeId = codeIdElement.value; 

            console.log(codeId);

            // Kiểm tra nếu codeId không rỗng
            if (codeId) {
                this.action = `/codes/${codeId}`; // Thiết lập action cho form
                this.submit(); // Gửi form
            } else {
                console.error('code ID is missing.'); // Ghi log lỗi nếu không tìm thấy ID
            }
        };
    });
    ...
    function closeDeleteCodeForm() {
        const showCodeForm = document.getElementById('showCodeForm')
        if (showCodeForm.style.display === 'none' || showCodeForm.style.display === '') {
            showCodeForm.style.display = 'block'; 
        } else {
            showCodeForm.style.display = 'none';
        }
    }
